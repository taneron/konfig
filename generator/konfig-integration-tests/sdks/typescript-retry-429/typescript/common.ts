...
import { readableStreamToString, TypescriptRetry429ClientError, parseIfJson } from "./error";
            ...
            if (e instanceof AxiosError && e.isAxiosError) {
                ...
                try {
                    const responseBody =
                        ...
                        : e.response?.data
                    throw new TypescriptRetry429ClientError(e, parseIfJson(responseBody), e.response?.headers)
                } catch (innerError) {
                    if (innerError instanceof ReferenceError) {
                        ...
                        throw new TypescriptRetry429ClientError(e, e.response?.data, e.response?.headers)
                    }
                    if (innerError instanceof TypescriptRetry429ClientError) {
                        // Got "TypescriptRetry429ClientError" from the above try block
                        throw innerError;
                    }
