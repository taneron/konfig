...
title: The Ultimate Guide to Using Postman to Document and Test Your API in 2023
authors: dylan
---
...
Let's briefly overview how Postman is organized and the basics of testing your
API in Postman.
...
We feel this description does not do Collections justice. Collections are more
...
Variables save a lot of time when you have common values across multiple
requests in your Collection. Try to always use Variables to organize your values
into one place and allow for easy parameterization of your requests.

<Figure src="/img/salesforce-platform-apis-variables.png" caption="The most popular API in the world has 17 variables configured for convenience (source: https://www.postman.com/state-of-api/api-global-growth/#most-popular-apis)"/>
...
Example requests should all live under the same Collection and should include
examples for every <Tooltip tip="An operation for an API is a combination of
HTTP Method and path, like 'GET /users'" text="operation"/> in an API. This
Collection is great for documenting your API for integration or as a forkable
Collection to make ad-hoc requests

Test requests live should live under a different Collection than your example
requests. Test Collections automatically test many requests or request flows at
the same time.
...
can be categorized under example requests.
...
#### Saving Multiple Example Requests

If you have multiple example requests for a particular
operation, you can add examples under a single operation. Multiple examples are
particularly helpful if your operation is <Tooltip text="polymorphic"
tip="accepts different data shapes"/>.
