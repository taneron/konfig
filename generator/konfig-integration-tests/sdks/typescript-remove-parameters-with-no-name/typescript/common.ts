...
import { readableStreamToString, TypescriptRemoveParametersWithNoNameClientError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new TypescriptRemoveParametersWithNoNameClientError(e, parseIfJson(responseBody), e.response?.headers)
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new TypescriptRemoveParametersWithNoNameClientError(e, e.response?.data, e.response?.headers)
                }
                if (innerError instanceof TypescriptRemoveParametersWithNoNameClientError) {
                    // Got "TypescriptRemoveParametersWithNoNameClientError" from the above try block
                    throw innerError;
                }
