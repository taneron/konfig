...
function generateSdkRepository(
  ...
  if (repoDescription === undefined) {
    result.reason =
      "Company meta description is too long. Must not be more than 350 characters.";
    return result;
  }
...
function generateRepositoryDescription(data: Published, language: string) {
  ...
  if (
    companyDescription.length < 350 &&
    companyDescription.length + description.length > 350
  )
    return companyDescription;
  else if (companyDescription.length > 350) return undefined;
