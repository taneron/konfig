...
title: The Ultimate Guide to Using Postman to Test Your API in 2023
authors: dylan
---
...
for collaboratively building APIs and especially provides amazing capabilities
for testing. [As organizations' investments of time and resources into APIs
increase](https://www.postman.com/state-of-api/outlook-for-apis-and-more/#api-investments-to-climb),
API-first organizations must leverage tools like Postman. Adopting an API
testing tool can save thousands of engineering hours and unnecessary Slack
messages.


<Admonition type="note">
There are alternatives to Postman like [Insomnia](https://insomnia.rest/) but
[Postman is still the most widely used
tool by a long margin](https://www.postman.com/state-of-api/tooling-for-apis-and-development/#api-platforms-and-tools).
</Admonition>

Today engineers are using Postman to [run 2,700+ tests on 100's of APIs on every
...
guide, we will cover fundamentals and dive into techniques to effectively test
...
This guide does assume that you have an API of basic CRUD complexity (i.e. you
can execute read/write operations to the API to modify the state that is
persisted in the backend). If you don't have an API, you can locally run our
simple API implementation to follow the guide yourself.
...
Let's briefly overview how Postman is organized and go over the basics of
...
That's enough about Postman's data model, **let's jump into real examples.**
...
[Variable](https://learning.postman.com/docs/sending-requests/variables/). Variables save a lot of time when you have common values amongst multiple
...
To use your variables, Postman provides a syntax for inserting variables using
curly braces. If you are familiar with templating engines like
...
variables can also be added to `Global` and `Environment` scopes.  Scopes help
define where a Variable is available to use. In general, we suggest putting
values used across Collections in the Global scope, security credentials in the
Environment scope, and other variables in the Collections scope.
...
tab to programmatically set Environment Variables.
...
job at prescribing how you organize your Requests and provides an intuitive UX
for your convenience. But Postman goes much further than just prescribing a way
to organize your requests for the sake of DRYness.
...
Postman Collections can be categorized into individual requests or standalone
tests.  Since Collections are the center of Postman, good hygiene when
organizing your requests is a good self-investment.

### Individual Requests

Often you will just want to execute a request to test individual operations. We
suggest that you organize individual requests into a separate collection so they
are isolated from collections used for testing. This way you have a place to
create ephemeral requests without affecting your test suite. This Collection of
individual requests is also a great way Collection for sharing examples with other
developers who are looking to integrate your API. Most public Collections are
structured this way with request examples for every operation in the API.

You can follow this flowchart for where to organize your requests.

```mermaid
flowchart LR
    Request --> OOR{Individual request?}
    OOR -->|Yes| OOC[Individual Request Collection]
    OOR -->|No| TC[Test Collection]
    OOC --> ER{Existing request?}
    ER -->|Yes| SAE[<a href='https://learning.postman.com/docs/sending-requests/examples/'>Save as example</a>]
    ER -->|No| CNR[Create new Request]
```
...
your Collection of individual requests. Postman will automatically create a
...
By running individual requests you can visually verify that the response returns
the expected value. This is super helpful for ad-hoc requests when debugging.
But it can be tough to verify a lot of requests at once, this is where
[Scripting](https://learning.postman.com/docs/writing-scripts/intro-to-scripts/)
comes in.

Say we want to create two automated assertions:
