...
import { readableStreamToString, TypescriptNoRelativeNpmLinkClientError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new TypescriptNoRelativeNpmLinkClientError(e, parseIfJson(responseBody), e.response?.headers)
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new TypescriptNoRelativeNpmLinkClientError(e, e.response?.data, e.response?.headers)
                }
                if (innerError instanceof TypescriptNoRelativeNpmLinkClientError) {
                    // Got "TypescriptNoRelativeNpmLinkClientError" from the above try block
                    throw innerError;
                }
