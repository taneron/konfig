...
<Tooltip tip="Software Development Kit">SDKs</Tooltip> makes the integration of <Tooltip
  tip="Representational state transfer"
  text="REST"
/> <Tooltip tip="Application Programming Interface">APIs</Tooltip> easier by packaging
<Tooltip
...
documentation code while API companies must provide continuous support for a
...
process.** SDKs heavily accelerate the time to complete an integration.

## So how does an SDK actually help?

Lets walkthrough an example in Python integrating Twilio's API without an SDK.
Then we will follow up with an implementation using their SDK. That way we can
concretely understand what the actual diff is between no SDK vs. SDK.  Lets
assume we are sending an SMS and later want to retrieve the SMS details.
...
```python without-sdk.py focus=3:12
```
...
Construct our authentication credentials according to how Twilio receives credentials and how the `requests` library
[can send it](https://requests.readthedocs.io/en/latest/user/authentication/#basic-authentication).
...
Make a `POST` request using the `requests` library and crossing our fingers 🤞 that
...
Make another request with the new URL while remembering to pass in the `auth`
...
Remove the URL variable as the SDK handles this for us.

~~_`url = f'https://api.twilio.com/2010-04-01/Accounts/{account_sid}/Messages.json'`_~~
...
Get rid of the manually constructed request body.
...
parameters. If we make a typo here the SDK will let us know before any HTTP
requests are made.
...
Replace our newly constructed URL with a simple method call
...
<CH.Section>
We distilled a couple setup steps into a single line. Most importantly as a
developer we do not need to [construct the URLs](focus://without-sdk.py#15) and
[form the request body](focus://without-sdk.py#18:22) to make an HTTP request.
...
Credentials were passed once and stored as part of the SDK. Notice that [we did not need to pass in an `auth` variable again](focus://without-sdk.py#31[48:56]).
...
The construction of the request body was handled so we [deleted a
variable](focus://without-sdk.py#18:22) and typos will be caught by the SDK when
calling `create()`.
...
Furthermore, SDKs integrate nicely with IDEs to reduce the chance of typos by
catching compilation or runtime errors earlier in the development process.
