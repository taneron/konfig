...
import { readableStreamToString, TypescriptSelfReferencingSchemaClientError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new TypescriptSelfReferencingSchemaClientError(e, parseIfJson(responseBody), e.response?.headers)
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new TypescriptSelfReferencingSchemaClientError(e, e.response?.data, e.response?.headers)
                }
                if (innerError instanceof TypescriptSelfReferencingSchemaClientError) {
                    // Got "TypescriptSelfReferencingSchemaClientError" from the above try block
                    throw innerError;
                }
