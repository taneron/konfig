...
Then we will follow up with an implementation using their SDK. Finally we will
analyze actual difference between implementations. Lets assume we are sending an
SMS and later want to retrieve the SMS details.
...
parameters. If we make a typo here the SDK will let us ahead of time.
...
<CH.Section>
We do not need to [construct the URLs](focus://without-sdk.py#15,30) and
...
variable](focus://without-sdk.py#18:22) and the SDK will catch incorrect
argument names when calling `create()`.
