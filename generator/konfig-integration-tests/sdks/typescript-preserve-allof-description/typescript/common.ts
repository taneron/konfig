...
import { readableStreamToString, TypescriptPreserveAllofDescriptionClientError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new TypescriptPreserveAllofDescriptionClientError(e, parseIfJson(responseBody), e.response?.headers)
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new TypescriptPreserveAllofDescriptionClientError(e, e.response?.data, e.response?.headers)
                }
                if (innerError instanceof TypescriptPreserveAllofDescriptionClientError) {
                    // Got "TypescriptPreserveAllofDescriptionClientError" from the above try block
                    throw innerError;
                }
