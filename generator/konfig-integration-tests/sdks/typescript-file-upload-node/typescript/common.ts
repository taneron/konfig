...
import { readableStreamToString, TypescriptFileUploadNodeClientError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new TypescriptFileUploadNodeClientError(e, parseIfJson(responseBody), e.response?.headers)
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new TypescriptFileUploadNodeClientError(e, e.response?.data, e.response?.headers)
                }
                if (innerError instanceof TypescriptFileUploadNodeClientError) {
                    // Got "TypescriptFileUploadNodeClientError" from the above try block
                    throw innerError;
                }
