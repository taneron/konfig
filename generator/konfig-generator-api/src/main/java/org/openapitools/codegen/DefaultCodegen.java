...
public class DefaultCodegen implements CodegenConfig {
    ...
    static {
        DefaultFeatureSet = FeatureSet.newBuilder()
                .includeDataTypeFeatures(
                        ...
                        DataTypeFeature.File, DataTypeFeature.Array, DataTypeFeature.Object, DataTypeFeature.Maps,
                        DataTypeFeature.CollectionFormat,
                        DataTypeFeature.CollectionFormatMulti, DataTypeFeature.Enum, DataTypeFeature.ArrayOfEnum,
                        ...
                        DataTypeFeature.MapOfEnum, DataTypeFeature.MapOfModel,
                ...
                .includeGlobalFeatures(
                        ...
                        GlobalFeature.Consumes, GlobalFeature.Produces, GlobalFeature.ExternalDocumentation,
     ...
     * API templates may be written multiple times; APIs are grouped by tag and the
     * file is written once per tag group.
     */
     ...
     * These get a different map of data bound to the templates. Supporting files
     * are written once.
     ...
     * This is used to model the parent hierarchy based on the 'allOf' composed
     * schemas.
     */
     ...
     * Some language generators also use class inheritance to implement the
     ...
     * Note: all language generators should support this to comply with the OAS
     * specification.
     */
     ...
     * This method is invoked when the input OpenAPI document has been parsed and
     * validated.
     */
    @Override
     ...
     * In case of union types this method has a key for each separate model and
     * import.
     *
     * @param name the name of the "Model"
    ...
    /**
     * Default constructor.
     * This method will map between OAS type and language-specified type, as well as
     * mapping
     * between OAS type and the corresponding import statement for the language.
     * This will
     ...
     * that value.
     * Otherwise, a name is constructed by creating a comma-separated list of all
     * the names
     ...
     * was originally developed for Java because the language does not have an
     * aliasing
     ...
     * This can be useful if one needs to change how values like null in string are
     * represnted
     *
     ...
     * vendor extensions
     * If they exist then cast them to java class instances and return them back in
     * a map
     *
     ...
     * the returned CodegenProperty may not be required and it may not be of type
     * string
     *
     ...
     *                                         additionalProperties schema not
     *                                         defining a required property, then
    ...
    public CodegenProperty fromProperty(String name, Schema p, boolean required,
        ...
        property.name = ("camelCase".equals(this.additionalProperties().get("objectPropertyNamingConvention")))
                ? new JavaClientCodegen().toVarName(name)
                : toVarName(name);
        ...
        setPropertyComposedSchemas(property, p, pDeref);
        // set example
...
//        }
        ...
        if (!ModelUtils.isArraySchema(p) && !ModelUtils.isMapSchema(p) && !isFreeFormObject(p)
             ...
             * not AnyType with nothing else set
             * so primitive schemas int, str, number, referenced schemas, AnyType schemas
             * with properties, enums, or composition
             */
    ...
    public CodegenOperation fromOperation(String path,
        ...
        if (operation.getResponses() != null && !operation.getResponses().isEmpty()) {
            ...
            for (Entry<String, ApiResponse> operationGetResponsesEntry : operation.getResponses().entrySet()) {
                ...
                if (r.vendorExtensions != null && r.vendorExtensions.get("x-do-not-generate") != null) {
                    LOGGER.info("Response {} for {} is not generated since it has the \"x-do-not-generate\" extension", key, operationId);
                    continue;
                }
        ...
        op.errorResponses = op.responses.stream().filter(response -> response.is4xx || response.is5xx).collect(Collectors.toList());
            ...
            if (contentType != null &&
                ...
                for (CodegenParameter cp : parametersFromBody) {
                    ...
                    if (cp.required) {
                        ...
                        optionalParamsWithRequestBodyProperties.add(cp);
                    }
                }
        ...
        for (CodegenParameter cp : allParams) {
            if (cp.required) { // required parameters
                ...
                if (!cp.isBodyParam)
                    requiredParamsWithRequestBodyProperties.add(cp.copy());
        ...
        op.allParamsWithRequestBodyProperties = allParamsWithRequestBodyProperties;
        ...
        op.defaultCanBeEmptyObject = !op.hasRequiredParams && (op.bodyParam == null || op.bodyParam.requiredVars.size() == 0);
    ...
    public CodegenParameter fromParameter(Parameter parameter, Set<String> imports) {
        ...
        if (codegenParameter.isQueryParam && codegenParameter.isDeepObject && loadDeepObjectIntoItems) {
            ...
            if (schema != null) {
                ...
                if (properties != null) {
                    codegenParameter.items.vars = properties.entrySet().stream()
                            .map(entry -> {
    ...
    public List<CodegenSecurity> fromSecurity(Map<String, SecurityScheme> securitySchemeMap) {
        ...
        // Assign isFirst field variable
        for (int i = 0; i < codegenSecurities.size(); i++) {
            CodegenSecurity codegenSecurity = codegenSecurities.get(i);
            codegenSecurity.isFirst = i == 0;
        }
     ...
     * value
     * of the 'additionalProperties' keyword. Some language generator use class
     * inheritance
     * to implement additional properties. For example, in Java the generated model
     * class
     ...
     * not
     * work for composed 'allOf' schemas. For example, in Java, if
     * additionalProperties
     * is set to true (which it should be by default, per OAS spec), then the
     * generated
     ...
     * This logic has been folded into the original call sites and long term will be
     * moved into
     ...
     * code. The file type can be "model-test",
     * "model-doc", "model", "api", "api-test", "api-doc", "supporting-mustache",
     * "supporting-common",
     ...
     * against the specification, potentially making
     ...
     * 3) additionalproperties is not defined, or additionalproperties: true, or
     * additionalproperties: {}.
     * <p>
     * Examples:
     * <p>
     ...
     * additional, undeclared
     * properties, that is, properties whose names are not listed in the properties
     * keyword.
     * The additionalProperties keyword may be either a boolean or an object.
     * If additionalProperties is a boolean and set to false, no additional
     * properties are allowed.
     * By default when the additionalProperties keyword is not specified in the
     * input schema,
     * any additional properties are allowed. This is equivalent to setting
     * additionalProperties
     ...
     * Into strings that can be rendered in the language that the generator will
     * output to
     */
     ...
     * Boolean/Schema/null
     * This will be used in openapi 3.1.0 spec processing to ensure that Booleans
     * become Schemas
     * Because our generators only understand Schemas
     * Note: use getIsBooleanSchemaTrue or getIsBooleanSchemaFalse on the
     * IJsonSchemaValidationProperties
     * if you need to be able to detect if the original schema's value was true or
     * false
     *
