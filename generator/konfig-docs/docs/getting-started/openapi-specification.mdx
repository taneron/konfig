...
But your OAS could be hosted somewhere else or come from a different Git repository.
...
Run `konfig init` to initialize your directory with the necessary
files to generate SDKs. It will a series of onboarding questions to bootstrap
your configuration.
...
<Admonition type="info" title="Sign up">
  Create an account with Konfig at https://dash.konfigthis.com/signup
</Admonition>

Run `konfig login` to setup credentials on your machine to run `konfig
generate`. This is necessary to run `konfig generate`.

```shell CLI
/my-sdks
❮ konfig login
Logging into https://api.konfigthis.com. Sign up at https://dash.konfigthis.com/signup
...
<Admonition type="info">
  ...
  setup Continuous Deployment through
  [polling](/docs/tutorials/automate-sdk-updates#polling) or [GitHub
  Action](/docs/tutorials/automate-sdk-updates#github-action). Sometimes its
  also necessary to add business logic in the SDKs which are also made easy by
  konfig through its [native support for customization](/docs/custom-sdks).
</Admonition>
