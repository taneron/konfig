...
import { readableStreamToString, TypescriptApiExceptionContainsResponseHeadersClientError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new TypescriptApiExceptionContainsResponseHeadersClientError(e, parseIfJson(responseBody), e.response?.headers)
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new TypescriptApiExceptionContainsResponseHeadersClientError(e, e.response?.data, e.response?.headers)
                }
                if (innerError instanceof TypescriptApiExceptionContainsResponseHeadersClientError) {
                    // Got "TypescriptApiExceptionContainsResponseHeadersClientError" from the above try block
                    throw innerError;
                }
