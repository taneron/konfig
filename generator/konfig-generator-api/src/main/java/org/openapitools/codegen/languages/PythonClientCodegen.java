...
public class PythonClientCodegen extends AbstractPythonCodegen {
    ...
    public String booleanFalseLiteral = "False";
    public String nullLiteral = "None";
...
//            supportingFiles.add(new SupportingFile("requirements." + templateExtension, "", "requirements.txt"));
        ...
        supportingFiles.add(new SupportingFile("operation_parameter_map." + templateExtension, packagePath(), "operation_parameter_map.py"));
        supportingFiles.add(new SupportingFile("api_response." + templateExtension, packagePath(), "api_response.py"));
        supportingFiles.add(new SupportingFile("exceptions_base." + templateExtension, packagePath(), "exceptions_base.py"));
        supportingFiles.add(new SupportingFile("type_util." + templateExtension, packagePath(), "type_util.py"));
        supportingFiles.add(new SupportingFile("validation_metadata." + templateExtension, packagePath(), "validation_metadata.py"));
        supportingFiles.add(new SupportingFile("client_custom." + templateExtension, packagePath(), "client_custom.py"));
        ...
        dependencies.add(new PythonDependency("urllib3", new String[]{"1.26.18", "2.0.0"}, new String[]{"~=", "~="},  new String[]{"^", "^"}));
    ...
    protected void generateEndpoints(OperationsMap objs) {
        ...
        for (CodegenOperation co: codegenOperations) {
            ...
            See https://youtrack.jetbrains.com/issue/PY-42137/PyCharm-type-hinting-doesnt-work-well-with-overload-decorator
             */
            // Dylan: I commented the next two liens because I just didn't personally see the use for it despite the comment
            // above. The type checking didn't even work in the first place anyways. And it just seemed to be extra
            // fluff.
            // Update: Nevermind I thought about it—just keep the .pyi file there is no harm
            ...
            ((HashMap<String, Object>) operations.get("additionalProperties")).entrySet().forEach((entry) -> {
                endpointTestMap.put(entry.getKey(), entry.getValue());
            });
        ...
        for (Map.Entry<String, PathItem> pathsEntry : paths.entrySet()) {
            ...
            String apiClassName = "".equals(toModelName(path)) ? "Root" : toModelName(path);
            pathEnumToApiClassname.put(pathEnumVar, apiClassName);
            pathModuleToApiClassname.put("".equals(toVarName(path)) ? "root" : toVarName(path), apiClassName);
        }
    ...
    public String toEnumVarName(String value, String datatype) {
        ...
        // prepend underscore if value starts with number
        usedValue = usedValue.matches("^[^_a-zA-Z].+") ? "_" + usedValue : usedValue;
        ...
        // dunderscore causes problem in Python
        if (usedValue.startsWith("__"))
            usedValue = usedValue.substring(1);

        // if used value starts with "_" but second character is valid then remove the "_" prefix
        if (usedValue.startsWith("_") && usedValue.length() > 1 && usedValue.substring(1).matches("^[_a-zA-Z].+"))
            usedValue = usedValue.substring(1);
    ...
    protected Object getObjectExample(Schema sc) {
        ...
        if (schema.getExtensions() != null) {
            Object example = schema.getExtensions().get("x-konfig-original-example");
            if (example != null) {
                return example;
            }
        }
    ...
    private String toExampleValueRecursive(String modelName, Schema schema, Object objExample, int indentationLevel, String prefix, Integer exampleLine, List<Schema> includedSchemas) {
        ...
        // I don't every want to make end-user import a model so I commented the section below
//        if (modelName != null) {
//            openChars = modelName + "(";
//            closeChars = ")";
//        }
        ...
        if (null != schema.get$ref()) {
            ...
            if (schema.getAnyOf() != null && schema.getAnyOf().size() > 0) {
                List<Schema> anyOf = schema.getAnyOf();
                Schema firstSchema = anyOf.get(0);
                return toExampleValueRecursive(getModelName(firstSchema), firstSchema, objExample, indentationLevel, prefix, exampleLine, includedSchemas);
            }

            if (schema.getAllOf() != null && schema.getAllOf().size() == 1) {
                List<Schema> allOf = schema.getAllOf();
                Schema firstSchema = allOf.get(0);
                return toExampleValueRecursive(getModelName(firstSchema), firstSchema, objExample, indentationLevel, prefix, exampleLine, includedSchemas);
            }
            ...
            if (example == null) {
                example = booleanTrueLiteral;
            } else {
                if ("false".equalsIgnoreCase(objExample.toString())) {
                    example = booleanFalseLiteral;
                } else {
                    example = booleanTrueLiteral;
                }
            }
            ...
            // Surround with dictinary syntax
            fullPrefix += objectOpenChar;
            closeChars = objectCloseChar;
            if (cycleFound) {
                return fullPrefix + closeChars;
            ...
            if (hasProperties) {
                ...
                // free form object, lets check if it has an example by checking x-konfig-generated-schema
                if (schema.getExtensions() != null && schema.getExtensions().get("x-konfig-generated-schema") != null) {
                    String generatedSchemaName = (String) schema.getExtensions().get("x-konfig-generated-schema");
                    Map<String, Schema> allDefinitions = ModelUtils.getSchemas(this.openAPI);
                    Schema refSchema = allDefinitions.get(generatedSchemaName);
                    if (null != refSchema) {
                        return exampleForObjectModel(refSchema, fullPrefix, closeChars, null, indentationLevel, exampleLine, closingIndentation, includedSchemas);
                    }
                } else {
    ...
    private String exampleForObjectModel(Schema schema, String fullPrefix, String closeChars, CodegenProperty discriminatorProperty, int indentationLevel, int exampleLine, String closingIndentation, List<Schema> includedSchemas) {
        ...
        for (Map.Entry<String, Schema> entry : requiredAndOptionalProps.entrySet()) {
            ...
            boolean isRequired = schema.getRequired() != null ? schema.getRequired().stream().anyMatch(requiredPropName -> requiredPropName.equals(entry.getKey())) : false;
            Schema propSchema = entry.getValue();
            boolean isParameter = false;
            List<String> isParameterList = null;
            if (schema.getExtensions() != null)
                isParameterList = (List<String>) schema.getExtensions().get("x-konfig-is-parameter");
            if (isParameterList != null) {
                isParameter = isParameterList.contains(propName);
            }
            propName = isParameter ? toParamName(propName) : toVarName(propName);
            String propModelName = null;
            Object propExample = null;
            if (discriminatorProperty != null && propName.equals(discriminatorProperty.name)) {
                propModelName = null;
                propExample = discriminatorProperty.example;
            } else {
                // Only generate an example if its required otherwise don't include it in any example code as its confusing and can cause problems
            ...
            if (propExample == null && !isRequired) continue;

            example += toExampleValueRecursive(propModelName,
                    propSchema,
                    propExample,
                    indentationLevel + 1,
                    "\"" + propName + "\"" + objectFieldDelimiter,
                    exampleLine + 1,
                    includedSchemas) + ",\n";
        }
    ...
    private Object exampleFromStringOrArraySchema(Schema sc, Object currentExample, String propName, boolean isRequired) {
        ...
        if (example != null) {
            ...
            // Only generate some BS example if this property is required
    ...
    public void setParameterExampleValue(CodegenParameter codegenParameter, Parameter parameter) {
        ...
        example = exampleFromStringOrArraySchema(schema, example, parameter.getName(), Optional.ofNullable(parameter.getRequired()).orElse(Boolean.FALSE));
    ...
    public void setParameterExampleValue(CodegenParameter codegenParameter, RequestBody requestBody) {
        ...
        example = exampleFromStringOrArraySchema(schema, example, codegenParameter.paramName, codegenParameter.required);
    ...
    public CodegenOperation fromOperation(String path,
        ...
        co.nickname = "".equals(pathModuleName) ? "root" : pathModuleName;
