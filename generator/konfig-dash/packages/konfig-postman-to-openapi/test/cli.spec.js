...
const itNotWindows = isWindows() ? it.skip : it
...
describe('Cli specs', function () {
  ...
  itNotWindows("should print an error when collection file doesn't exist", async function () {
  ...
  itNotWindows('should print an error when result file cannot be created', async function () {
  ...
  itNotWindows('should print an error when "options" file doesn\'t exist', async function () {
    await rejects(execa('node', [cliPath, COLLECTION_SIMPLE, '-o', './no_exist/options.json']), {
      name: 'Error',
      stderr:
        'Error: invalid "options" parameter -> ENOENT: no such file or directory, open \'./no_exist/options.json\'',
      exitCode: 1,
    })
  })

  itNotWindows('should print an error when options file is not a valid json', async function () {
    await rejects(
      execa('node', [cliPath, COLLECTION_SIMPLE, '-o', INVALID_OPTIONS_INFO]),
      (err) => {
  ...
  itNotWindows('should print correctly help command', async function () {
