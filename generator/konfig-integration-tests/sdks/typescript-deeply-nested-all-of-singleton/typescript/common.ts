...
import { readableStreamToString, TypescriptDeeplyNestedAllOfSingletonClientError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new TypescriptDeeplyNestedAllOfSingletonClientError(e, parseIfJson(responseBody), e.response?.headers)
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new TypescriptDeeplyNestedAllOfSingletonClientError(e, e.response?.data, e.response?.headers)
                }
                if (innerError instanceof TypescriptDeeplyNestedAllOfSingletonClientError) {
                    // Got "TypescriptDeeplyNestedAllOfSingletonClientError" from the above try block
                    throw innerError;
                }
