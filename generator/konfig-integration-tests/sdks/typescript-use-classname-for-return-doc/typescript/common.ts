...
import { readableStreamToString, TypescriptUseClassnameForReturnDocClientError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new TypescriptUseClassnameForReturnDocClientError(e, parseIfJson(responseBody), e.response?.headers)
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new TypescriptUseClassnameForReturnDocClientError(e, e.response?.data, e.response?.headers)
                }
                if (innerError instanceof TypescriptUseClassnameForReturnDocClientError) {
                    // Got "TypescriptUseClassnameForReturnDocClientError" from the above try block
                    throw innerError;
                }
