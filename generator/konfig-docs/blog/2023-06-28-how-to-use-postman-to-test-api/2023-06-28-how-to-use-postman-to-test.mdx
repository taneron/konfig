...
Lets briefly overview how Postman is organized. Postman's data model revolves
...
Variables are crucial when you have common values amongst multiple requests in
your Collection. For example referencing the same User ID across multiple
requests.

#### Scope

Variables live in a [Scope](https://learning.postman.com/docs/sending-requests/variables/#variable-scopes).
...
If your API needs authorization.
...
the "Pre-request Script" tab to programatically set Environment Variables.
...
are separated from collections used for tests as you'll see in the following
sections.

#### Generate Collection from OAS (Recommended<Flame/>)

Generating your collections from an OAS can save a lot of time. Postman will
