...
import { readableStreamToString, TypescriptOptionalApiKeyError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new TypescriptOptionalApiKeyError(e, parseIfJson(responseBody), e.response?.headers)
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new TypescriptOptionalApiKeyError(e, e.response?.data, e.response?.headers)
                }
                if (innerError instanceof TypescriptOptionalApiKeyError) {
                    // Got "TypescriptOptionalApiKeyError" from the above try block
                    throw innerError;
                }
