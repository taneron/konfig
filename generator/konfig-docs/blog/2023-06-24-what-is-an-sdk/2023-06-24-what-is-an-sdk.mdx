...
Making HTTP requests were converted to ergonomic method calls. The construction
of the request body was handled so we [deleted a
variable](focus://without-sdk.py#18:22) and [removed any notion of `POST` and
`GET` requests](focus://without-sdk.py#25[26:29],31[30:32]). The SDK also would
have easily caught incorrect argument names when calling `create()`.
...
Furthermore, SDKs catching compilation or runtime errors earlier in the
development process.
