...
import { readableStreamToString, TypescriptMultipleSecuritiesForSingleParameterThatShouldBeSecurityClientError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new TypescriptMultipleSecuritiesForSingleParameterThatShouldBeSecurityClientError(e, parseIfJson(responseBody), e.response?.headers)
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new TypescriptMultipleSecuritiesForSingleParameterThatShouldBeSecurityClientError(e, e.response?.data, e.response?.headers)
                }
                if (innerError instanceof TypescriptMultipleSecuritiesForSingleParameterThatShouldBeSecurityClientError) {
                    // Got "TypescriptMultipleSecuritiesForSingleParameterThatShouldBeSecurityClientError" from the above try block
                    throw innerError;
                }
