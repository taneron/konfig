...
import { readableStreamToString, TypescriptRelativeNpmLinksClientError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new TypescriptRelativeNpmLinksClientError(e, parseIfJson(responseBody), e.response?.headers)
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new TypescriptRelativeNpmLinksClientError(e, e.response?.data, e.response?.headers)
                }
                if (innerError instanceof TypescriptRelativeNpmLinksClientError) {
                    // Got "TypescriptRelativeNpmLinksClientError" from the above try block
                    throw innerError;
                }
