...
examples for every <Tooltip tip="An operation for an API is a combination of HTTP Method and path, like 'GET /users'" text="operation"/> in an API.
This Collection is great for documenting your API for integration or as a
forkable Collection to make ad-hoc requests
...
them. Examples are particularly helpful if your operation is <Tooltip text="polymorphic" tip="accepts different data shapes"/>,
which can make it hard to document. Examples are not only helpful for how to
 make valid requests, but also common invalid requests.

 Here ish

<VideoSection>
1.
Click the **Save as Example** button after making a request
</VideoSection>
