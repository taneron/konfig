...
import { readableStreamToString, NewscatcherError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new NewscatcherError(e, parseIfJson(responseBody), e.response?.headers)
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new NewscatcherError(e, e.response?.data, e.response?.headers)
                }
                if (innerError instanceof NewscatcherError) {
                    // Got "NewscatcherError" from the above try block
                    throw innerError;
                }
