...
import { readableStreamToString, TypescriptArrayQueryStringClientError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new TypescriptArrayQueryStringClientError(e, parseIfJson(responseBody), e.response?.headers)
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new TypescriptArrayQueryStringClientError(e, e.response?.data, e.response?.headers)
                }
                if (innerError instanceof TypescriptArrayQueryStringClientError) {
                    // Got "TypescriptArrayQueryStringClientError" from the above try block
                    throw innerError;
                }
